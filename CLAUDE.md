...
npm run db:clear-pantry  # DELETE all pantry_items (stop dev server first if the DB file was replaced while running)
npm run icons:pwa      # Regenerate public/icons/icon-*.png from public/icons/icon.svg (after editing the SVG)
```

**PWA / Android:** Chrome expects **192×192 and 512×512 PNG** icons in the web manifest (SVG-only is unreliable). Those PNGs are generated by `icons:pwa` and committed under `public/icons/`. The service worker registers in **production** over HTTPS; for local HTTPS PWA testing, set `NEXT_PUBLIC_PWA_DEV=1` in `.env.local`.
